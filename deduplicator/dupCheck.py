...
class ArchChecker(object):
	...
	def simplePhashCheck(self):
		...
		for fileN, fileCtnt in self.arch:
			dummy_fName, dummy_hexHash, pHash, dHash, imX, imY = self.hashModule.hashFile(self.archPath, fileN, fileCtnt.read())
	...
	def localBKPhash(self, dirPath):
		...
		for fileN, fileCtnt in self.arch:
			dummy_fName, dummy_hexHash, pHash, dummy_dHash, dummy_imX, dummy_imY = self.hashModule.hashFile(self.archPath, fileN, fileCtnt.read())
			pHash = int(pHash, 2)
	...
	def addNewArch(self, shouldPhash=True):
		...
		for fName, fp in archIterator:
			...
			fCont = fp.read()
			try:
				fName, hexHash, pHash, dHash, imX, imY = self.hashModule.hashFile(self.archPath, fName, fCont, shouldPhash=shouldPhash)
